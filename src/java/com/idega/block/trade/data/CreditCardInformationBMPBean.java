...
public class CreditCardInformationBMPBean extends GenericEntity implements CreditCardInformation {
	...
	public Object ejbFindByMerchant(String merchantPK, String merchantType) throws FinderException {
		Table table = new Table(this);

		SelectQuery query = new SelectQuery(table);
		query.addColumn(table, getIDColumnName());
		query.addCriteria(new MatchCriteria(table.getColumn(COLUMN_MERCHANT_PK), MatchCriteria.EQUALS, merchantPK));
		query.addCriteria(new MatchCriteria(table.getColumn(COLUMN_TYPE), MatchCriteria.EQUALS, merchantType));
